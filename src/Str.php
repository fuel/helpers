...
class Str
{
	...
	public static function random(string $type = 'alnum', int $length = 16): string
	{
		...
		{
			case 'basic':
				return $randomizer->getInt(0, PHP_INT_MAX);
			case 'unique': // depricate !
			case 'md5':
				return md5($randomizer->getBytes(128));
			case 'sha1' :
				return sha1($randomizer->getBytes(128));
			case 'uuid':
				$pool = $randomizer->shuffleArray(['8', '9', 'a', 'b']);
			...
			default:
				...
				for ($i=0; $i < $length; $i++)
				{
					$str .= substr($pool, $randomizer->getInt(0, strlen($pool) -1), 1);
				}
				return $str;
		}
	}
	...
	{
		// any JSON string must be longer than 2 characters.
		if (strlen($string) < 2)
		{
			return false;
		}
	...
	public static function is_xml(string $string): bool
	{
		...
		{
			throw new RuntimeException('libxml is required to use Str::is_xml()');
		}
